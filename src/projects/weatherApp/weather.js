...
const ErrorDisplay = ({ error }) => <p className="error">{error}</p>;
...
const Weather = () => {
  ...
  const [location, setLocation] = useState({ lat: "44.34", long: "10.99" });
  ...
  const fetchWeatherData = async () => {
    try {
      const response = await fetch(
        `${weatherApi.baseUrl}?lat=${location.lat}&lon=${location.long}&appid=${weatherApi.key}&units=metric`
      );
      const data = await response.json();
      dispatch({ type: actionTypes.FETCH_SUCCESS, payload: data });
    } catch (error) {
